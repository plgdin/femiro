...
export async function handleAdminUsers(req, body = {}) {
    ...
    if (!targetId || !['user', 'employee', 'admin'].includes(role))
        return json(400, { error: 'Invalid user or role.' });
    ...
    if (authError || !updatedUser.user)
        return json(500, { error: authError?.message || 'Could not update auth role.' });
