...
export async function handleCreateOrder(req: any, body: any) {
  ...
  if (!lines.length || !address) throw new Error('Cart and delivery address are required.')
  ...
  if (!products || products.length !== ids.length) throw new Error('A cart item is no longer available.')

  const productMap = new Map(products.map(product => [Number(product.id), product]))
  const items = lines.map(line => {
    const product = productMap.get(line.id)
    if (!product || product.in_stock === false) throw new Error(`${product?.name || 'Item'} is unavailable.`)
    if (product.sizes?.length && line.size && !product.sizes.includes(line.size)) throw new Error('Invalid product size.')
    return {
  ...
  if (coupon) {
    ...
    if (discountRow) {
      ...
      if (eligibleSubtotal <= 0 || eligibleSubtotal < Number(discountRow.min_spend || 0)) throw new Error('This discount does not apply to the cart.')
  ...
  const { data: localOrder, error: insertError } = await supabaseAdmin
    ...
    .select('id')
    .single()
  if (insertError || !localOrder) throw insertError || new Error('Could not create order.')
